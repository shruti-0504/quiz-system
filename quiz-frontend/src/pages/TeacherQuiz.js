...
const TeacherQuiz = () => {
    ...
    const validateDuration = (duration, startTime, endTime) => {
        ...
        return value < 0 ? 0 : maxDuration > 0 && value > maxDuration ? maxDuration : value;
    };
    ...
    const addQuestion = () => {
        setNewQuiz({
            ...newQuiz,
            questions: [...newQuiz.questions, { question: "", options: ["", "", "", ""], correctOption: 0 }],
        });
    };
    ...
    return (
        <div className="teacher-quiz-container">
            ...
            <div className="button-group spaced">
                <button onClick={() => setShowCreateForm(!showCreateForm)}>Create Quiz</button>
                <button onClick={() => setShowAllotForm(!showAllotForm)}>Allot Quiz</button>
            </div>

            {showCreateForm && (
                <div className="create-quiz-form">
                    <h3>Create New Quiz</h3>
                    <input type="text" placeholder="Quiz Title" value={newQuiz.title}
                        onChange={(e) => setNewQuiz({ ...newQuiz, title: e.target.value })} />
                    <input type="text" placeholder="Course Code" value={newQuiz.courseCode}
                        onChange={(e) => setNewQuiz({ ...newQuiz, courseCode: e.target.value })} />
                    <input type="text" placeholder="Quiz Password" value={newQuiz.password}
                        onChange={(e) => setNewQuiz({ ...newQuiz, password: e.target.value })} />
                    <input type="datetime-local" placeholder="Start Time" value={newQuiz.startTime}
                        onChange={(e) => setNewQuiz({ ...newQuiz, startTime: e.target.value })} />
                    <input type="datetime-local" placeholder="End Time" value={newQuiz.endTime}
                        onChange={(e) => setNewQuiz({ ...newQuiz, endTime: e.target.value })} />
                    <input type="number" placeholder="Duration (in minutes)" value={newQuiz.duration}
                        onChange={(e) => setNewQuiz({ 
                            ...newQuiz, 
                            duration: validateDuration(e.target.value, newQuiz.startTime, newQuiz.endTime) 
                        })} />

                    {newQuiz.questions.map((q, qIndex) => (
                        <div className="question-card" key={qIndex}>
                            <input type="text" placeholder={`Question ${qIndex + 1}`} value={q.question}
                                onChange={(e) => updateQuestionText(qIndex, e.target.value)} />
                            {q.options.map((opt, optIndex) => (
                                <input key={optIndex} type="text" placeholder={`Option ${optIndex + 1}`} value={opt}
                                    onChange={(e) => updateOption(qIndex, optIndex, e.target.value)} />
                            ))}
                            <label>Correct Option:</label>
                            <select value={q.correctOption} onChange={(e) => updateCorrectOption(qIndex, e.target.value)}>
                    ...
                    <button className="spaced-button" onClick={addQuestion}>Add Question</button>
                    <button className="spaced-button" onClick={handleCreateQuiz}>Submit Quiz</button>
                </div>
            )}
            ...
            {showAllotForm && (
                <div className="allot-quiz-form">
                    ...
                    <input type="text" placeholder="Section" value={allotSection}
