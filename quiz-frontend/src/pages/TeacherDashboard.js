...
const TeacherDashboard = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:5000/students/no-section")
            .then(res => res.json())
    ...
    return (
        <div>
            ...
            {students.map(student => (
                <div key={student._id}>
                    <h3>{student.email}</h3>
                    <input type="text" placeholder="Section" onChange={(e) => student.section = e.target.value} />
                    <button onClick={() => updateSection(student._id, student.section)}>Update</button>
                </div>
            ))}
        </div>
    );
